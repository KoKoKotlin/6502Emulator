...
abstract class AddressMode() {
    ...
    abstract fun additionalCycle(param: Word, isWrite: Boolean): Boolean
}
...
object ZeroPageIndexedX: MemMode() {
    ...
    override fun additionalCycle(param: Word, isWrite: Boolean) = false
}
...
object AbsoluteX : MemMode() {
    ...
    override fun additionalCycle(param: Word, isWrite: Boolean) =
...
object IndexedIndirectX : MemMode() {
    override fun getAddr(param: Word) =
        wordFromBytes(
            mem.read((param wordPlus cpu.X) wordMod 256),
            mem.read((param wordPlus cpu.X wordPlus 1) wordMod 256)
        )
...
object IndexedIndirectY : MemMode() {
    override fun getAddr(param: Word) =
        wordFromBytes(
            mem.read(param),
            mem.read((param wordPlus 1) wordMod 256)
        ) wordPlus cpu.Y
...
object ImpliedX : AddressMode() {
    override fun read(param: Word) = cpu.X
    override fun getAddr(param: Word) = 0x00.toWord()
    override fun additionalCycle(param: Word, isWrite: Boolean) = false
...
object ImpliedY : AddressMode() {
    override fun read(param: Word) = cpu.Y
    override fun getAddr(param: Word) = 0x00.toWord()
    override fun additionalCycle(param: Word, isWrite: Boolean) = false
